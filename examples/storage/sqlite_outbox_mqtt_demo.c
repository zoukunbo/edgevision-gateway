 ...
 */
static mqtt_publisher_result_t publish_real(const pending_item_t *item,
                                            int broker_port) {
  measurement_t measurement;
  if (measurement_from_json(item->payload, strlen(item->payload),
                            &measurement) != MEASUREMENT_JSON_OK)
  ...
  if (n < 0 || (size_t)n >= sizeof(expected_topic) ||
      strcmp(expected_topic, item->topic) != 0)
...
int main(int argc, char **argv) {
  ...
  if (!deliver && !unavailable && !status) {
    fprintf(stderr,
            "Usage: %s deliver-mqtt|deliver-unavailable|status DATABASE\n",
            argv[0]);
    return EXIT_FAILURE;
  }
  if (!MQTT_OUTBOX_EXERCISE_READY) {
    fputs("EXERCISE INCOMPLETE: fill Q1-Q3, then set "
          "MQTT_OUTBOX_EXERCISE_READY=1. No database or MQTT opened.\n",
          stderr);
    return EXIT_FAILURE;
  }
  sqlite3 *db = NULL;
  int rc = sqlite3_open_v2(argv[2], &db,
                           (deliver || unavailable) ? SQLITE_OPEN_READWRITE
                                                    : SQLITE_OPEN_READONLY,
                           NULL);
  if (rc == SQLITE_OK)
    rc = (deliver || unavailable)
             ? deliver_pending_mqtt(db, unavailable ? 1884 : 1883)
             : print_status(db);
  else
    fprintf(stderr, "database open failed: rc=%d %s\n", rc, sqlite3_errstr(rc));
