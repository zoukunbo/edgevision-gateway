...
static int seed_pending(sqlite3 *db, const char *json) {
  ...
  if (rc == SQLITE_OK)
    rc = sqlite3_prepare_v2(
        db, "INSERT INTO outbox(measurement_id,topic) VALUES (?1,?2);", -1,
        &stmt, NULL);
  if (rc == SQLITE_OK)
    rc = sqlite3_bind_int64(stmt, 1, measurement_id);
  if (rc == SQLITE_OK)
...
static int load_oldest_pending(sqlite3 *db, outbox_item_t *item) {
  ...
  int rc = sqlite3_prepare_v2(db,
                              ...
                              " WHERE o.state='pending' ORDER BY o.id LIMIT 1;",//outbox 的 id 从小到大排序。
                              -1, &stmt, NULL);
  if (rc == SQLITE_OK)
    rc = sqlite3_step(stmt);
  if (rc == SQLITE_DONE)
    rc = DELIVERY_NO_PENDING;
  if (rc == SQLITE_ROW) {
...
static int mark_sent(sqlite3 *db, sqlite3_int64 outbox_id) {
  ...
  int rc = sqlite3_prepare_v2(
      db, "UPDATE outbox SET state='sent' WHERE id=?1 AND state='pending';", -1,
      &stmt, NULL);
  if (rc == SQLITE_OK)
  ...
  /**
   * sqlite3_changes() 用来获取：
   *    当前这个 sqlite3 *db 连接上，最近一次 INSERT / UPDATE / DELETE 影响了多少行。
   */
...
int main(int argc, char **argv) {
  ...
  if (!DELIVERY_EXERCISE_READY) {
    fputs("EXERCISE INCOMPLETE: fill D1-D3, then set "
          "DELIVERY_EXERCISE_READY=1. No database opened.\n",
          stderr);
    return EXIT_FAILURE;
  }
  char json[4096] = {0};
  ...
  sqlite3 *db = NULL;
  if (rc == SQLITE_OK)
    rc = sqlite3_open_v2(argv[2], &db,
                         status ? SQLITE_OPEN_READONLY
                                : SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE,
                         NULL);
  if (rc == SQLITE_OK && !status)
    rc = initialize_schema(db);
  if (rc == SQLITE_OK && seed)
    rc = seed_pending(db, json);
  if (rc == SQLITE_OK && (fail || ok))
    rc = deliver_one(db, ok);
  if (rc == SQLITE_OK && status)
    rc = print_status(db);
  if (rc != SQLITE_OK && rc != DELIVERY_NOT_CONFIRMED &&
      rc != DELIVERY_NO_PENDING)
    fprintf(stderr, "operation failed: rc=%d %s\n", rc, sqlite3_errstr(rc));
  int close_rc = sqlite3_close(db);
  if (seed && rc == SQLITE_OK)
    puts("SEEDED one pending Outbox item.");
